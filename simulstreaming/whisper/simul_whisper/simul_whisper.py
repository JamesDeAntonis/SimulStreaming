...
class PaddedAlignAttWhisper:
    def __init__(self, cfg: AlignAttConfig) -> None:
        ...
        if use_half and self.model.device.type == "cuda":
            ...
            if use_half:
                logger.warning(
                    "Half precision requested but not available on CPU, using float32"
                )
        ...
        if cfg.use_compile:
            try:
                ...
                if hasattr(torch, "compile"):
                    try:
                        ...
                        self.model.encoder = torch.compile(
                            self.model.encoder, mode="reduce-overhead"
                        )
                        logger.info(
                            "Compiled model encoder with torch.compile() for faster inference"
                        )
                ...
                logger.warning(
                    f"Error during torch.compile(): {e}. Continuing without compilation."
                )
        ...
        def kv_hook(module: torch.nn.Linear, _, net_output: torch.Tensor):
            if (
                module.cache_id not in self.kv_cache
                or net_output.shape[1] > self.max_text_len
            ):
                # save as-is, for the first token or cross attention
                self.kv_cache[module.cache_id] = net_output
            else:
                x = self.kv_cache[module.cache_id]
                self.kv_cache[module.cache_id] = torch.cat(
                    [x, net_output], dim=1
                ).detach()
    ...
    def refresh_segment(self, complete=False):
        ...
        if not complete and len(self.segments) > 2:
            logger.debug(
                "keeping last two segments because they are and it is not complete."
            )
    ...
    def _current_tokens(self):
        ...
        if not self.context.is_empty():
            context_toks = self.context.as_tensor_beam(
    ...
    def infer(self, is_last=False):
        ...
        generation_progress = []
        generation = {
            "starting_tokens": BeamTokens(
                current_tokens[0, :].clone(), self.cfg.beam_size
            ),
        ...
        while (
            ...
            print(
                f"Calling logits with shape {tokens_for_logits.shape}: {round(time.time() - t0, 3)}"
            )

            logits = self.logits(
                tokens_for_logits, encoder_feature
            )  # B, len(tokens), token dict size
            ...
            generation_progress_loop.append(
                ("beam_tokens", Tokens(current_tokens[:, -1].clone()))
            )
            ...
            attn_of_alignment_heads = median_filter(
                attn_of_alignment_heads, 7
            )  # from whisper.timing
            ...
            # Store tensor instead of .tolist() - convert only if needed later
            generation_progress_loop.append(
                ("most_attended_frames", most_attended_frames.clone())
            )
            # Only convert to list if DEBUG logging is enabled (avoid GPU sync otherwise)
            if logger.isEnabledFor(logging.DEBUG):
            ...
            if (
                not is_last
                and self.last_attend_frame - most_attended_frame
    ...
    def logdir_save(self, input_segments, new_hypothesis, generation):
        ...
        with open(
            os.path.join(dir, f"iter_{self.logdir_i:05d}_hypothesis.txt"), "w"
        ) as f:
            if generation:
